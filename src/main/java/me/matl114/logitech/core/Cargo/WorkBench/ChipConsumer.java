...
public class ChipConsumer extends AbstractMachine {
    ...
    public ChipConsumer(ItemGroup category, SlimefunItemStack item, RecipeType recipeType, ItemStack[] recipe,
        ...
        setDisplayRecipes(Utils.list(
                ...
                AddUtils.getInfoShow("&f取反","&7插入%s进行取反操作".formatted(Language.get("item.antilogic.name")),
                        "&7运算规则:","&7依次对输入芯片的01位进行'~'运算","&8⇨ &7~0=1","&8⇨ &7~1=0"),LogiTechSlimefunItemStacks.NOLOGIC,
                AddUtils.getInfoShow("&f左移","&7插入%s进行左移操作".formatted(Language.get("item.existence.name")),
                        "&7运算规则:","&7对01码的首位加入0,丢弃最末尾"),LogiTechSlimefunItemStacks.EXISTE,
                AddUtils.getInfoShow("&f右移","&7插入%s进行右移操作".formatted(Language.get("item.uniqueness.name")),
                        "&7运算规则:","&7当最末位是1时,向01码最末尾部加1,并丢掉首位","&7当最末位是0时,向01码最末尾部加0,并丢掉首位"),LogiTechSlimefunItemStacks.UNIQUE
        ));
    }
    protected final ItemCounter[] MATCH_ITEM=new ItemCounter[]{
           CraftUtils.getConsumer( LogiTechSlimefunItemStacks.NOLOGIC),
            CraftUtils.getConsumer(  LogiTechSlimefunItemStacks.EXISTE),
            CraftUtils.getConsumer(  LogiTechSlimefunItemStacks.UNIQUE),
    };
