...
public class Storages {
    ...
    public static  void disableNetworkProxy(){
        try{
            ((CustomSlimefunItem) LogiTechSlimefunItems.QUANTUM_LINK).addDisplayRecipe(AddUtils.getInfoShow("&c警告!",
                    "&7由于网络拓展未知错误,该物品已经不兼容网络存储",
                    "&7请联系服务器管理员检查网络版本",
                    "&7如果你认为这是一个偶发性错误或者损坏的物品导致的问题,请重启服务器",
                    "&7如果你认为这是网络版本问题,请更换网络版本以重新兼容该物品"));
            ((CustomSlimefunItem)LogiTechSlimefunItems.QUANTUM_LINK).addDisplayRecipe(null);
        }catch (Throwable es){
            Debug.logger(es);
        }
    }
