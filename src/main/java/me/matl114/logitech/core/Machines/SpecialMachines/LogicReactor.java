...
public class LogicReactor extends AbstractProcessor {
        ...
        this.setDisplayRecipes(
                Utils.list(
                        AddUtils.getInfoShow("&f机制",
                                ...
                                "&7这些槽位用于输入布尔组件,即 %s 和 %s".formatted(Language.get("item.boolean_true.name"),Language.get("item.boolean_false.name")),
                                "&7只有这些槽位的物品满足一定机制时,合成才会尝试进行"),null,
                        AddUtils.getInfoShow("&f机制",
                                "&7机器还有一个&c正常输入槽&7和&e正常输出槽&7,分别位于第三排的&e左侧&7和&e右侧空格,分别用于输入所需的 [%s] 和输出产物 ".formatted(Language.get("item.logic_gate.name")),
                                "&7机器中间一个开关,用于调控机器是否会进行合成进程,当点击关闭时,当前进程将被强制终止",
                                "&7当机器为开启状态且为空闲,输出槽非满,四对布尔输入槽&c非空&7且满足右侧某个输入条件时",
                                "&7才会尝试&e消耗&7正常输入槽内的 [%s] 进行合成".formatted(Language.get("item.logic_gate.name")),
                        ...
                        ),LogiTechSlimefunItemStacks.LOGIC_GATE

                )
        );
        this.machineRecipes=new ArrayList<>(){{
            add(MachineRecipeUtils.FromMachine(AddUtils.formatInfoMachineRecipe(Utils.array(LogiTechSlimefunItemStacks.LOGIC),tick,
                    "&7当四对输入槽物品均为同样的布尔组件时","&7将会尝试生成 %s".formatted(Language.get("item.logic.name")))));
            add(MachineRecipeUtils.FromMachine(AddUtils.formatInfoMachineRecipe(Utils.array(LogiTechSlimefunItemStacks.NOLOGIC),tick,
                    "&7当四对输入槽物品均为相反的布尔组件时","&7将会尝试生成 %s".formatted(Language.get("item.antilogic.name")))));
            add(MachineRecipeUtils.FromMachine(AddUtils.formatInfoMachineRecipe(Utils.array(LogiTechSlimefunItemStacks.UNIQUE),tick,
                    "&7当四对输入槽物品中只有一对布尔组件相异","&7将会尝试生成 %s".formatted(Language.get("item.uniqueness.name")))));
            add(MachineRecipeUtils.FromMachine(AddUtils.formatInfoMachineRecipe(Utils.array(LogiTechSlimefunItemStacks.EXISTE),tick,
                    "&7当前三个材料的生成条件均不满足","&7将会尝试生成 %s".formatted(Language.get("item.existence.name")))));
        }};
    }
    ...
    public int checkLogic(BlockMenu inv,int[] inputs){
        ...
        for (int i=0;i<4;++i){
            ...
            if (it!=null){
                ...
                if(CraftUtils.matchItemStack(it, LogiTechSlimefunItemStacks.BOOLEAN_TRUE,false)){
                    code1=2*code1+1;
                    ++cnt;
                }else if (CraftUtils.matchItemStack(it, LogiTechSlimefunItemStacks.BOOLEAN_FALSE,false)){
                    code1=2*code1;
                }else {
                    cnt=-64;
                }
    ...
    protected Pair<MachineRecipe, ItemConsumer[]> findNextRecipe(BlockMenu inv,int[] inputs,int[] outputs){
        ...
        if(iv!=null&&CraftUtils.matchItemStack(iv, LogiTechSlimefunItemStacks.LOGIC_GATE,false)&&(ov==null||ov.getAmount()<ov.getMaxStackSize())){
