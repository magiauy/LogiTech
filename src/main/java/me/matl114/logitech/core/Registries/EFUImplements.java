...
public class EFUImplements {
    ...
    public static final EquipmentFU DEMO = new SimpleFU(getNameKey("demo_fu"),EquipmentFU.FURarity.COMMON, EquipmentSlot.HAND){

    };
    public static final EquipmentFU MINER = new SimplePotionEffectEquipmentFU("攻速强化",getNameKey("att-speed-fu"),EquipmentFU.FURarity.COMMON, 10,16,List.of(Triplet.of("haste",1,0.5f)),ANYHAND);
    ...
    public static class SimplePotionEffectEquipmentFU extends SimpleFU{
        ...
        public ItemStack getInfoDisplay(){
            ...
            effects.forEach(effectProvider -> {
                infos.add("⇨ §b%s : §7".formatted(CraftUtils.getEffectName(effectProvider.getEffect()))+(effectProvider.getBaseLevel()==0?"":"%d+".formatted(effectProvider.getBaseLevel()))+".1f*<单元等级>".formatted(effectProvider.getAmplifier()));
            });
